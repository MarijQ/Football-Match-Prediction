...
class DDA():
    ...
    def ML_load(self, fraction):
        ...
        self.df = self.df.sample(fraction=fraction, withReplacement=False, seed=42)
    ...
    def preprocess_data(self):
        ...
        indexers = [StringIndexer(inputCol=col, outputCol=f"{col}_index").fit(self.df) for col in categorical_cols]

        # Apply OneHotEncoder to the indexed categorical features
        encoders = [OneHotEncoder(inputCols=[indexer.getOutputCol()], outputCols=[f"{col}_vec"]) for col, indexer in zip(categorical_cols, indexers)]
    ...
    def train_models(self):
        ...
        paramGrid_lr = ParamGridBuilder().addGrid(lr.regParam, [0.1, 0.01]).addGrid(lr.elasticNetParam, [0.0, 0.5, 1.0]).build()
        paramGrid_dt = ParamGridBuilder().addGrid(dt.maxDepth, [2, 5, 10]).build()
        paramGrid_rf = ParamGridBuilder().addGrid(rf.maxDepth, [5, 10]).addGrid(rf.maxBins, [32, 64]).build()
        ...
        crossval_lr = CrossValidator(estimator=lr, estimatorParamMaps=paramGrid_lr, evaluator=evaluator_rmse, numFolds=3)
        crossval_dt = CrossValidator(estimator=dt, estimatorParamMaps=paramGrid_dt, evaluator=evaluator_rmse, numFolds=3)
        crossval_rf = CrossValidator(estimator=rf, estimatorParamMaps=paramGrid_rf, evaluator=evaluator_rmse, numFolds=3)

        models_and_params = {'LinearRegression': crossval_lr, 'DecisionTree': crossval_dt, 'RandomForest': crossval_rf}
    ...
    def ML_non_distributed(self, fraction):
        # Load data
        ...
        categorical_transformer = SklearnOneHotEncoder(handle_unknown='ignore')
        ...
        models = [
            ...
            ('DecisionTree', SklearnDecisionTreeRegressor(random_state=42)),
            ('RandomForest', SklearnRandomForestRegressor(random_state=42))
        ]
    ...
    def full(self):
        ...
        fractions = [1.0]
        non_distributed_runtimes = []
        num_rows = [46000 * fraction for fraction in fractions]

        for fraction in fractions:
            # start_time = time.time()
            # self.ML_non_distributed(fraction)
            # end_time = time.time()
            # non_distributed_runtime = end_time - start_time
            # non_distributed_runtimes.append(non_distributed_runtime)

            start_time = time.time()
            self.ML_load(fraction)
            self.preprocess_data()
            self.train_test_split()
            self.train_models()
            end_time = time.time()
            distributed_runtime = end_time - start_time
            distributed_runtimes.append(distributed_runtime)

        # print(non_distributed_runtimes, distributed_runtimes)
        ...
        # plt.xlabel('Log Number of Rows (sampled data)')
